...
namespace PeinRecoilRework.Config.Settings
{
    public class PistolWeaponRecoilSettings
    {
        ...
        public static void Bind(ConfigFile Config, int order, string category)
        {
            ...
            AimingMultiplier = Config.Bind(formattedCategory, "Aiming Multiplier", 1f, new ConfigDescription("Roll visual recoil multiplier while aiming", new AcceptableValueRange<float>(0.1f, 10f), new ConfigurationManagerAttributes { Order = 990 }));
            TimeMultiplier = Config.Bind(formattedCategory, "Speed Multiplier", 1f, new ConfigDescription("Roll visual recoil speed multiplier", new AcceptableValueRange<float>(0.1f, 10f), new ConfigurationManagerAttributes { Order = 980 }));
        }
    }
}
