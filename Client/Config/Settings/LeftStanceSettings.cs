...
namespace PeinRecoilRework.Config.Settings
{
    public class LeftStanceSettings
    {
        ...
        public static void Bind(ConfigFile Config, int order, string category)
        {
            ...
            LeftStanceOffset = Config.Bind(formattedCategory, "Left Stance Offset", 0.1f, new ConfigDescription("Offset for the left stance position.", new AcceptableValueRange<float>(0f, 0.2f), new ConfigurationManagerAttributes { Order = 900 }));
            ...
            LeftStanceSpeed = Config.Bind(formattedCategory, "Left Stance Speed", 8f, new ConfigDescription("Speed at which the weapon moves when transitioning shoulders.", new AcceptableValueRange<float>(1f, 10f), new ConfigurationManagerAttributes { Order = 880 }));
        }
    }
}
