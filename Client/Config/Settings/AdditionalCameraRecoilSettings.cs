...
namespace PeinRecoilRework.Config.Settings
{
    public class AdditionalCameraRecoilSettings
    {
        ...
        public static void Bind(ConfigFile Config, int order, string category)
        {
            ...
            FastSpringRealRecoilVerticalMinMax = Config.Bind(formattedCategory, "Fast Spring Real Recoil Vertical Min/Max", new Vector2(0.3f, 0.6f), new ConfigDescription("Multiplier range for vertical recoil. X = minimum, Y = maximum.", null, new ConfigurationManagerAttributes { Order = 847 }));
